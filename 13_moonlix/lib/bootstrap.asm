...
@@: create_idt_descriptor INTERRUPT_NULL
    ...
    ; Тот случай, когда надо создать задачу
...
; Очистка системной памяти с 0x00200000 .. [3 Мб]
CLEAR_memory:

	; 0x2000000 - 0x3FFFFF (верхние 2Мб)
    mov edi, 0x00200000
    mov ecx, 0x00200000 shr 2
    xor eax, eax
    rep stosd
    ret    
...
VM_initialize:
    ...
    mov [es:TSS_VM + 0x20], dword 0 ; по умолчанию, VM указывает на базовый обработчик (но его нет)
...
I80_log:
	...
	; edi = cursor_bootlog*160
	; esi = par1
	
	imul edi, [glob.cursor_bootlog], 80*2 
	add  edi, 0xB8000
	mov  esi, [par1]

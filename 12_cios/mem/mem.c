...
/*
 * Выделение памяти на куче [HEAP...] для системных целей
 * Выделенная память не удаляется более
 */
...
void mem_paging_init()
{
    ...
    // Основная страница, которая располагается в первых 4к памяти
    data_pdbr = malloc(4096); for (i = 0; i < 4096; i++) data_pdbr[i] = 0;
...
void mem_init()
{
    ...
    mem_sys = malloc(65536);

    mem_keyb_pressed = malloc(256);
    mem_keyb_buffer  = malloc(512);

    // Вектор со списком задач
    data_sys_task = (SysTask*) malloc(4096);
}